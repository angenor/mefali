...
- [X] T045 [P] Validation quickstart : SC-001..009 backend couverts par les tests d'intégration (`cargo test`) ; **passe manuelle K3 sur émulateur EXÉCUTÉE** (Android API 36, coursier +2250700000003, course simulée Tantie) — login OTP, écran K3 (nom prestataire), mode dégradé online (COLLECTÉ + EN_LIVRAISON + incident + 3 événements, géoloc réelle), parcours OFFLINE (coupe réseau → validation empreinte/distance locale + coche optimiste + file, serveur intact) puis retour réseau → **drain automatique < 5 s, idempotent**. Un bug d'auto-sync (tempête de rebuilds connectivity_plus) trouvé ET corrigé pendant la passe.
