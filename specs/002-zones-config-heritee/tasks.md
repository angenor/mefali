...
- [ ] T003 [P] Types publics du crate : `backend/crates/zones/src/modele.rs` (Zone, Devise, ConfigurationEffective + ValeurProvenance, CategorieActive, ErreurZones — data-model §5) et exports dans `backend/crates/zones/src/lib.rs` ; dépendances du crate (`socle`, sqlx, serde, uuid, chrono, thiserror, async-trait) dans `backend/crates/zones/Cargo.toml`. (½ j)
- [ ] T004 [P] Déclarer AVANT implémentation (constitution VI) les 3 événements dans `docs/taxonomie-evenements.md` : `zone.parametre_modifie`, `categorie.forcage_change`, `categorie.activation_changee` (payloads de research R9 / data-model §3). (½ j)
