...
- [X] T040 Dérouler `specs/005-prestataires-catalogue-vendeur/quickstart.md` intégralement : parcours curl §3 (agrément, suspension/rétablissement à jeton constant, prix barré refusé, seeds ×2) + validation émulateur §4 (V1 trois états, V2 liste/fiche/aperçu, suspension à chaud) ; vérifier transversalement SC-009 (auteur/source/horodatage sur chaque événement) et SC-011 (aucun payload avec nom/contact/GPS). — FAIT : curl §3 vert (SC-001/002/003/006/007/008/012 + SC-009/SC-011 prouvés en base), émulateur §4 vert (V1 SC-004 « ouvert hors horaires → fermé », V2 liste/fiche/aperçu client, suspension à chaud → refus i18n « Bascule refusée » puis rétablissement à jeton constant). A RÉVÉLÉ 1 défaut fonctionnel (bascule sans site → événement mensonger, corrigé) + 3 défauts UI (horaires « aujourd'hui » doublé, libellé bascule rogné, refus de bascule silencieux — tous corrigés + tests).
- [X] T041 Chaîne qualité complète : `cargo test` workspace + `cargo sqlx prepare` sans diff + `./scripts/generate-clients.sh` puis `git diff --exit-code clients/ openapi.json` + `dart analyze` (JAMAIS `flutter analyze`) et `flutter test` dans `apps/mefali_pro` et `apps/packages/mefali_core` + contrôle de dérive des `.g.dart` (build_runner sans diff). — FAIT : `cargo test` workspace 47 suites vertes (247 tests) ; `cargo sqlx prepare --check` exit 0 ; clients régénérés déterministes (2 passes identiques, seul diff = les 2 descriptions utoipa 404 enrichies + manifeste FILES corrigé — l'écart CONTRAT « stubs test périmés » du commit 51d634a est résolu) ; `dart analyze` propre sur les 3 apps ; `flutter test` 41 (pro) + 72 (core) verts ; dérive `.g.dart` corrigée (`mes_articles.g.dart` du commit 94be055 régénéré).
- [X] T042 Revue Definition of Done (`docs/user-stories-v2.md` §0.4) story par story pour VND-01, VND-02, VND-03, VND-04 : (1) critères d'acceptation couverts par des tests, (2) annotations utoipa à jour + clients régénérés sans diff manuel, (3) migration versionnée + seeds à jour, (4) événement outbox pour tout changement d'état + taxonomie MET-01 déclarée, (5) clés i18n fr externalisées, (6) paramètres « paramétrables » en configuration de zone — consigner l'écart éventuel et le corriger AVANT le commit de clôture du cycle. — FAIT : revue consignée dans `revue-dod-phase8.md` (8 défauts corrigés dont 1 fonctionnel bloquant ; 5 réserves de couverture de test + i18n mineurs consignés en dette ; 4 décisions assumées). Les six critères §0.4 sont satisfaits, réserves documentées.
