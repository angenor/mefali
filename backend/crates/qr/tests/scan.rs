...
async fn scan_dans_le_rayon_collecte_et_evenement(pool: sqlx::PgPool) {
    ...
    let r = bac
        .qr
        .collecter(bac.coursier, demande_scan(arrets[0], &jeton, SITE_LAT + 0.00027, SITE_LON))
        .await
    ...
    assert!(ev[0].get("site_lat").is_none() && ev[0].get("lat").is_none(), "ARTCI : pas de GPS brut");
}
...
async fn scan_hors_rayon_refuse(pool: sqlx::PgPool) {
    ...
    let e = bac
        .qr
        .collecter(bac.coursier, demande_scan(arrets[0], &jeton, SITE_LAT + 0.0027, SITE_LON))
        .await
        .unwrap_err();
    assert_eq!(e.message_cle(), Some("hors_zone"));
    assert_eq!(bac.statut_arret(arrets[0]).await, "a_collecter", "0 collecte à distance");
...
async fn mauvais_vendeur_refuse(pool: sqlx::PgPool) {
    ...
    let e = bac
        .qr
        .collecter(bac.coursier, demande_scan(arrets[0], &jeton_kofi, SITE_LAT, SITE_LON))
        .await
        .unwrap_err();
...
async fn gating_en_livraison_avec_arret_indisponible(pool: sqlx::PgPool) {
    ...
    let r = bac
        ...
        .await
        .unwrap();
    assert!(r.en_livraison, "tous les arrêts résolus (1 collecté + 1 indisponible)");
...
async fn rejeu_idempotent(pool: sqlx::PgPool) {
    ...
    let r1 = bac.qr.collecter(bac.coursier, demande.clone()).await.unwrap();
    ...
    assert_eq!(r1, r2);
    assert_eq!(bac.evenements("arret.collecte").await.len(), 1, "aucune double collecte");
}
